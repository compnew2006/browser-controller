...
function corsHeaders(req, pinnedOrigin) {
 ...
 */
function isAllowedOrigin(req, pinnedOrigin) {
    const origin = req.headers.origin;
    if (!origin)
        return { ok: true, pinnedOrigin };
    ...
    if (!pinnedOrigin)
        return { ok: true, pinnedOrigin: origin };
 ...
 */
export function findListenersOnPort(port) {
...
export class ExtensionBridge {
    ...
    tryListen() {
        return new Promise((resolve, reject) => {
            ...
            const pickAuthProtocol = (protocols) => {
                for (const p of protocols) {
                    if (p.startsWith(AUTH_PROTOCOL_PREFIX) || p === 'bc-auth')
                        return 'bc-auth';
                }
                return false;
            };
    ...
    sendControl(type, payload = {}) {
        if (!this.isConnected())
            return; // extension gone — nothing to notify
        try {
    ...
    waitForConnection(timeoutMs = 10_000) {
        if (this.isConnected())
    ...
    async callTool(tool, params, sessionId, signal, agentName) {
        if (!this.isConnected()) {
            try {
                await this.waitForConnection(5_000);
            }
            catch {
                throw new Error('Chrome extension not connected. Make sure the Real Browser MCP extension is installed and enabled.');
            }
        }
        return this.sendToolCall(tool, params, 0, sessionId, signal, agentName);
    }
    sendToolCall(tool, params, retryCount, sessionId, signal, agentName) {
