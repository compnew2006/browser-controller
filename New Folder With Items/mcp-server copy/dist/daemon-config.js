...
export function loadOrCreateToken() {
    ...
    const created = { token: crypto.randomBytes(24).toString('hex'), createdAt: Date.now() };
 ...
 */
export function loadOrCreateEnrollment() {
    try {
    ...
    const created = { secret: crypto.randomBytes(24).toString('hex'), createdAt: Date.now() };
    fs.mkdirSync(STATE_DIR, { recursive: true });
...
export function readEnrollmentSecret() {
    try {
        const raw = fs.readFileSync(ENROLLMENT_FILE, 'utf8');
        const parsed = JSON.parse(raw);
