...
import { DEFAULT_WS_HOST, DEFAULT_WS_PORT, DAEMON_INFO_FILE, IPC_SOCKET_PATH, STATE_DIR, TOKEN_FILE, loadOrCreateEnrollment, loadOrCreateToken, } from './daemon-config.js';
...
class Daemon {
    ...
    handleNewClient(socket) {
        ...
        socket.on('close', () => {
            if (client) {
                ...
                const stillAlive = Array.from(this.clients.values()).some((c) => c !== client && c.agentName === name);
                if (!stillAlive) {
    ...
    handleHttp(req, url) {
        ...
        if (path === '/kill') {
            ...
            if (!sid)
                return { ok: false, error: 'sessionId query param required' };
