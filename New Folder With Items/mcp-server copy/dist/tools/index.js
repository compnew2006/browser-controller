 ...
 */
const idempotentToolNames = new Set(allTools.filter((t) => t.idempotent === true).map((t) => t.name));
export function isIdempotent(tool) {
 ...
 */
const timeoutByToolName = new Map(allTools
    .filter((t) => typeof t.timeoutMs === 'number')
    .map((t) => [t.name, t.timeoutMs]));
export function toolTimeoutMs(tool) {
