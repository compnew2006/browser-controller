...
describe('daemon enrollment gate (closes first-contact TOFU race)', { timeout: 30_000 }, () => {
  // The shared daemon above already created enrollment.json. `enrollment` was
  // read from disk in startDaemon(). These tests prove that even if a hostile
  // extension wins the Origin-pin race, it still cannot obtain the token
  // because it does not know the enrollment secret.

  it('/pair WITHOUT the X-BC-Enrollment header is rejected (403)', async () => {
    const res = await httpGet('/pair', { headers: { 'X-BC-Enrollment': '' }, raw: true });
    expect(res.status).toBe(403);
    // And it must NOT contain the token in the body.
    expect(res.body).not.toContain(token);
  });

  it('/pair WITH a WRONG enrollment secret is rejected (403)', async () => {
    const res = await httpGet('/pair', { headers: { 'X-BC-Enrollment': 'wrong-secret' }, raw: true });
    expect(res.status).toBe(403);
    expect(res.body).not.toContain(token);
  });

  it('/pair WITH the CORRECT enrollment secret returns the token', async () => {
    const res = await httpGet('/pair', { headers: { 'X-BC-Enrollment': enrollment }, raw: true });
  ...
  it('/status and /kill also require the enrollment secret (no bypass via other routes)', async () => {
    // Without the header, /status must be rejected too — not just /pair.
    const statusNoKey = await httpGet('/status', { headers: { 'X-BC-Enrollment': '' }, raw: true });
    expect(statusNoKey.status).toBe(403);

    // With a wrong secret, /kill must also be rejected.
    const killWrongKey = await httpGet('/kill?sessionId=s1', { headers: { 'X-BC-Enrollment': 'wrong' }, raw: true });
    expect(killWrongKey.status).toBe(403);

    // With the correct secret, /status works (proving the gate passes legit use).
    const statusOk = await httpGet('/status', { headers: { 'X-BC-Enrollment': enrollment }, raw: true });
